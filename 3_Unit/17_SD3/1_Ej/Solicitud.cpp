...
char * Solicitud::doOperation(char *ip, int puerto, int operationId, char *arguments, int args_size)
{
	...
	// VARIABLE PARA ALMACENAR LOS ARGUMENTOS RECIBIDOS, EN ESTE CASO UNA ESTRUCTURA TIMEVAL
	char *argumentos_respuesta = (char *) malloc(sizeof(struct timeval));

	// COPIAMOS LOS ARGUMENTOS EN LA VARIABLE
	memcpy(argumentos_respuesta, men_res->arguments, sizeof(struct timeval));
